...
struct extract_type_info
{
    ...
    {
        return (is_pod_struct<Type>::value ? when_not_found<Type>() : addInfo(encode_type<Type>(), sizeof(Type), alignof(Type))),
    ...
    template <class Type>
    constexpr std::size_t when_not_found()
        CPP_11_NO_CONST_WARN_OFF noexcept;
    constexpr std::size_t addInfo(
        std::size_t type_id,
        std::size_t size = 0,
        std::size_t align_of = 0) const noexcept
    {
        return meta_info_ref.type_sizes[meta_info_ref.index] = size,
...
template <class Type>
constexpr std::size_t extract_type_info<Nth, FieldCount, InsertBraces>::when_not_found() noexcept
{
...
{
    return get_full_type_info<T, decltype(info), info.size(), InsertBraces>(std::make_index_sequence<Fields>{}, info), info;
}
